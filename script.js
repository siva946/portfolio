...
function renderSkills() {
  ...
  portfolioData.skills.forEach(skill => {
    container.innerHTML += `
      <div class="col-6 col-md-3 col-lg-2 text-center skill-item">
    ...
    row.innerHTML += `
      <div class="card col">
        ...
        <div class="card-body d-flex flex-column">
          ...
          <a href="${project.link}" target="_blank" class="button mt-auto">${project.link === '#' ? 'Live demo' : 'Get Link'}</a>
        </div>
      </div>
    `;
  });
...
function handleFormSubmit(event) {
  ...
  if (isValid) {
    emailjs.sendForm(CONFIG.emailjs.serviceId, CONFIG.emailjs.templateId, form)
      .then(() => {
        showToast('Thank you for contacting us, ' + nameInput.value.trim() + '!', 'success');
        form.reset();
      }, (error) => {
...
function animateProjects() {
  ...
  function checkProjectsInView() {
    rows.forEach((row, index) => {
      ...
      if (rect.top < window.innerHeight - 50 && !row.classList.contains('animated')) {
        row.classList.add(index % 2 === 0 ? 'appear-left' : 'appear-right', 'animated');
      }
    });
  }
...
function initParticles() {
  ...
  particlesJS('particles-js', {
    particles: {
      ...
      line_linked: { enable: true, distance: 150, color: isDark ? "#ffffff" : "#181825", opacity: 0.4, width: 1 },
